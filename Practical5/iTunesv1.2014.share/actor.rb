...
class Actor
  ...
  # It returns a array of song objects.
	#
	# I've added in unless to check if the the owner is nil, and if so it will be skipped
	# owner is nil when the song id doesn't match with any ids of the owner

  def what_songs_does_he_own()
      $songs.select do |song|
				unless song.owners.nil?
					song.owners.include?(@name)
				end
			end
  end
